-- ============================================
-- CORRECCIÓN: FÓRMULA DE UTILIDAD EN VISTA
-- vw_eventos_analisis_financiero_erp
-- ============================================
-- FÓRMULA CORRECTA:
-- UTILIDAD REAL = Ingresos - Gastos - Provisiones (con IVA)
-- UTILIDAD BRUTA = Ingresos - Gastos - Provisiones (sin IVA, subtotales)
-- ============================================
...
SELECT -- IDENTIFICACIÓN
    ...
    -- FÓRMULA: UTILIDAD_BRUTA = INGRESOS_SUBTOTAL - GASTOS_SUBTOTAL - PROVISIONES_SUBTOTAL
    -- ════════════════════════════════════════════════════════════════════
    ...
    CASE
        WHEN COALESCE(ing.ingresos_subtotal, 0) > 0 THEN (
            (
                COALESCE(ing.ingresos_subtotal, 0) - COALESCE(gas.gastos_subtotal, 0) - COALESCE(prov.provisiones_subtotal, 0)
...
FROM evt_eventos_erp e -- JOIN Cliente
    LEFT JOIN evt_clientes_erp c ON e.cliente_id = c.id -- JOIN Estado
    LEFT JOIN evt_estados_erp est ON e.estado_id = est.id -- JOIN Tipo Evento
    ...
    ) gas ON true -- SUBQUERY Provisiones
    LEFT JOIN LATERAL (
