-- ============================================
-- VISTA vw_eventos_analisis_financiero_erp
-- Reformulada con los campos correctos
-- ============================================
...
SELECT
  ...
  -- PROVISIONES POR CATEGORÍA - Combustible (clave: COMB)
  COALESCE(prov.provision_combustible, 0) AS provision_combustible,
  -- PROVISIONES POR CATEGORÍA - Materiales (clave: MAT)
  COALESCE(prov.provision_materiales, 0) AS provision_materiales,
  -- PROVISIONES POR CATEGORÍA - RH (clave: RH)
  COALESCE(prov.provision_rh, 0) AS provision_rh,
  -- PROVISIONES POR CATEGORÍA - SPs (clave: SP o NULL)
  COALESCE(prov.provision_sps, 0) AS provision_sps,

  -- UTILIDAD CALCULADA
  COALESCE(ing.ingresos_totales, 0) - COALESCE(gas.gastos_totales, 0) AS utilidad_real,
  CASE
    WHEN COALESCE(ing.ingresos_totales, 0) > 0
    THEN ((COALESCE(ing.ingresos_totales, 0) - COALESCE(gas.gastos_totales, 0)) / COALESCE(ing.ingresos_totales, 0)) * 100
    ELSE 0
