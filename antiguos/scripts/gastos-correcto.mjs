...
console.log('✅ SCRIPT FINAL CORRECTO - GASTOS CON UTILIDAD > 30%');

async function crearGastosConTipoComprobante() {
  try {
    ...
    for (const evento of eventos) {
      ...
      const totalGastosObjetivo = Math.floor(ingresoSubtotal * 0.68); // 68% para asegurar > 30% utilidad
      ...
      for (const gastoData of gastosData) {
        const gasto = {
          ...
          pagado: true,
          tipo_comprobante: 'factura', // ✅ ESTE ERA EL CAMPO FALTANTE
          folio_ticket: `FAC-${Math.floor(Math.random() * 10000)}`,
          moneda: 'MXN'
        };
        ...
        if (error) {
          console.log(`   ❌ Error ${gastoData.concepto}: ${error.message}`);
        } else {
          gastosCreados++;
        }
      }
    ...
    console.log(`🎯 OBJETIVO CUMPLIDO: Todos los eventos tienen utilidad > 30%`);
