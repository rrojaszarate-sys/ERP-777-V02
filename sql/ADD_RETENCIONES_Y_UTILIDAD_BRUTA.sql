-- ============================================
-- ADD: Retenciones y Utilidad Bruta
-- ============================================
--
-- CAMBIOS:
-- 1. Agregar columnas de retención a evt_gastos_erp
-- 2. Agregar columnas de retención a evt_ingresos_erp
-- 3. Agregar columnas de retención a evt_provisiones_erp
-- 4. Actualizar vista con utilidad_bruta basada en subtotales
--
-- CONCEPTO:
-- - Utilidad Bruta = Ingresos Subtotal - Gastos Subtotal - Provisiones Subtotal
-- - NO incluye IVA ni retenciones (son flujo de efectivo)
-- - Las retenciones se guardan para cálculos de flujo de efectivo
-- ============================================

-- ============================================
-- PASO 1: Agregar columnas de retención a gastos
-- ============================================
ALTER TABLE evt_gastos_erp
ADD COLUMN IF NOT EXISTS retencion_iva DECIMAL(12,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS retencion_isr DECIMAL(12,2) DEFAULT 0;

COMMENT ON COLUMN evt_gastos_erp.retencion_iva IS 'Retención de IVA aplicada al gasto';
COMMENT ON COLUMN evt_gastos_erp.retencion_isr IS 'Retención de ISR aplicada al gasto';

-- ============================================
-- PASO 2: Agregar columnas de retención a ingresos
-- ============================================
ALTER TABLE evt_ingresos_erp
ADD COLUMN IF NOT EXISTS retencion_iva DECIMAL(12,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS retencion_isr DECIMAL(12,2) DEFAULT 0;

COMMENT ON COLUMN evt_ingresos_erp.retencion_iva IS 'Retención de IVA aplicada al ingreso';
COMMENT ON COLUMN evt_ingresos_erp.retencion_isr IS 'Retención de ISR aplicada al ingreso';

-- ============================================
-- PASO 3: Agregar columnas de retención a provisiones
-- ============================================
ALTER TABLE evt_provisiones_erp
ADD COLUMN IF NOT EXISTS retencion_iva DECIMAL(12,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS retencion_isr DECIMAL(12,2) DEFAULT 0;

COMMENT ON COLUMN evt_provisiones_erp.retencion_iva IS 'Retención de IVA estimada';
COMMENT ON COLUMN evt_provisiones_erp.retencion_isr IS 'Retención de ISR estimada';

-- ============================================
-- PASO 4: Recrear vista con utilidad bruta
-- ============================================
...
SELECT
  ...
  -- ============================================
  -- INGRESOS (con totales, subtotales y retenciones)
  -- ============================================
  ...
  COALESCE(ing.ingresos_retencion_iva, 0) AS ingresos_retencion_iva,
  COALESCE(ing.ingresos_retencion_isr, 0) AS ingresos_retencion_isr,
  COALESCE(ing.ingresos_cobrados, 0) AS ingresos_cobrados,
  COALESCE(ing.ingresos_pendientes, 0) AS ingresos_pendientes,

  -- ============================================
  -- GASTOS (con totales, subtotales y retenciones)
  -- ============================================
  ...
  COALESCE(gas.gastos_retencion_iva, 0) AS gastos_retencion_iva,
  COALESCE(gas.gastos_retencion_isr, 0) AS gastos_retencion_isr,
  ...
  -- ============================================
  -- PROVISIONES (con totales, subtotales y retenciones)
  -- ============================================
  ...
  COALESCE(prov.provisiones_retencion_iva, 0) AS provisiones_retencion_iva,
  COALESCE(prov.provisiones_retencion_isr, 0) AS provisiones_retencion_isr,
  ...
  -- ============================================
  -- TOTAL EGRESOS SUBTOTAL (sin IVA ni retenciones)
  -- ============================================
  (
  ...
  -- ============================================
  (
    COALESCE(gas.gastos_retencion_iva, 0) + COALESCE(gas.gastos_retencion_isr, 0) +
    COALESCE(prov.provisiones_retencion_iva, 0) + COALESCE(prov.provisiones_retencion_isr, 0)
  ) AS total_retenciones_egresos,
  ...
  -- ============================================
  -- UTILIDAD BRUTA = SUBTOTALES (SIN IVA NI RETENCIONES)
...
-- SUBQUERY Ingresos (con subtotales y retenciones)
LEFT JOIN LATERAL (
  SELECT
    SUM(total) AS ingresos_totales,
    SUM(COALESCE(subtotal, total / 1.16)) AS ingresos_subtotal,
    SUM(COALESCE(iva, total - (total / 1.16))) AS ingresos_iva,
    SUM(COALESCE(retencion_iva, 0)) AS ingresos_retencion_iva,
    SUM(COALESCE(retencion_isr, 0)) AS ingresos_retencion_isr,
...
-- SUBQUERY Gastos (con subtotales y retenciones)
LEFT JOIN LATERAL (
  SELECT
    ...
    SUM(COALESCE(retencion_iva, 0)) AS gastos_retencion_iva,
    SUM(COALESCE(retencion_isr, 0)) AS gastos_retencion_isr,
...
-- SUBQUERY Provisiones (con subtotales y retenciones)
LEFT JOIN LATERAL (
  SELECT
    ...
    SUM(COALESCE(p.retencion_iva, 0)) AS provisiones_retencion_iva,
    SUM(COALESCE(p.retencion_isr, 0)) AS provisiones_retencion_isr,
...
SELECT
  ...
  gastos_totales,
  gastos_subtotal,
  gastos_retencion_iva,
  gastos_retencion_isr,
  provisiones_total,
