...
export const DocumentosEvento: React.FC<DocumentosEventoProps> = ({ eventoId, estadoActual, onDocumentUploaded }) => {
  ...
  // En desarrollo, comprobar si el usuario existe en core_users
  const [effectiveUserId, setEffectiveUserId] = useState<string | null>(null);

  // Verificar si el usuario existe en core_users al montar el componente
  React.useEffect(() => {
    const checkUser = async () => {
      if (isDevMode) {
        // Intentar encontrar un usuario válido en core_users
        ...
        if (devUser?.id) {
          ...
          console.warn('No se encontró un usuario de desarrollo válido en core_users');
          toast.error('No hay usuarios válidos en core_users');
        }
      } else {
  ...
  const handleUpload = async (tipo: string, file: File) => {
    ...
    // Verificar que el usuario existe en core_users
    ...
    if (userError || !userExists) {
      console.error('Error: Usuario no encontrado en core_users');
