...
type Evento = Database['public']['Tables']['eventos_erp']['Row'];
type Estado = Database['public']['Tables']['estados_erp']['Row'];
...
export class WorkflowService {
  ...
  async changeEventState(
    ...
    // Solo agregar updated_by si el usuario existe en users_erp
    if (userExists) {
      updateData.updated_by = userId;
      workflowLogger.info(`Usuario ${userId} existe en users_erp, se actualizará updated_by`);
    } else {
      workflowLogger.warn(`Usuario ${userId} NO existe en users_erp, se omitirá updated_by`);
    }
  ...
  async getEventWorkflowHistory(eventId: number): Promise<any[]> {
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          usuario:users_erp(nombre, email)
        `)
