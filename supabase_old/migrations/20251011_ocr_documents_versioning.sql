-- =====================================================
-- MIGRACIÓN: Sistema de Documentos OCR con Versionado
-- =====================================================
-- Crea tabla para almacenar documentos procesados con OCR
-- incluyendo sistema de versionado automático
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_evt_documentos_ocr_evento_id
  ON evt_documentos_ocr(evento_id);

CREATE INDEX IF NOT EXISTS idx_evt_documentos_ocr_estado
  ON evt_documentos_ocr(estado_procesamiento)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_evt_documentos_ocr_fecha_documento
  ON evt_documentos_ocr(fecha_documento)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_evt_documentos_ocr_establecimiento
  ON evt_documentos_ocr(establecimiento)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_evt_documentos_ocr_version
  ON evt_documentos_ocr(evento_id, nombre_archivo, version);

-- Índice para búsqueda en JSON
CREATE INDEX IF NOT EXISTS idx_evt_documentos_ocr_datos_extraidos
...
-- Insertar datos de ejemplo (comentado, descomentar si se necesita)
/*
INSERT INTO evt_documentos_ocr (
  evento_id,
  nombre_archivo,
  archivo_path,
  ...
  'ticket_oxxo_ejemplo.jpg',
  'evento-123/gastos/1234567890-v1-ticket_oxxo_ejemplo.jpg',
  'completed',
  'google_vision',
  'OXXO\nTotal: $117.00\nFecha: 01/10/2025',
  92,
  '{"establecimiento":"OXXO","total":117.00,"fecha":"2025-10-01"}',
